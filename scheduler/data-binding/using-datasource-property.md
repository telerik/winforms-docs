...
Here's a dataset design view for the SchedulerData.mdf database file that ships with Progress&reg; Telerik&reg; UI for WinForms. You can see all the fields that can be defined for appointments and resources. Also notice a "join table" named "TransientAppointmentsResources" that assigns a particular resource to an appointment.
