...
To use `RadTaskBoard` when working with NuGet packages, install the `Telerik.UI.for.WinForms.AllControls` package. The [package target framework version may vary]({%slug winforms-available-nugets%}).
...
<snippet id='task-board-taskboardgettingstarted-gettingstarted-cs' />
<snippet id='task-board-taskboardgettingstarted-gettingstarted-vb' />
