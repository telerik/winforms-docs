...
description: Check our AI Assistant documentation article for the RadSpreadsheet WinForms control.
...
```C#
this.radSpreadsheet1.IsAIEnabled = true;
```
```VB.NET
Me.radSpreadsheet1.IsAIEnabled = True
```

Once enabled, you can show or hide the AI panel using the `IsAIPanelVisible` property or by calling the `ToggleAIPanel()` method.

```C#
// Show the AI panel
this.radSpreadsheet1.IsAIPanelVisible = true;

// Or toggle the panel visibility
this.radSpreadsheet1.ToggleAIPanel();
```
```VB.NET
' Show the AI panel
Me.radSpreadsheet1.IsAIPanelVisible = True

' Or toggle the panel visibility
Me.radSpreadsheet1.ToggleAIPanel()
```

### Using the Ribbon Button

When using the `RadSpreadsheetRibbonBar`, you can display an **AI Assistant** toggle button that allows users to show or hide the AI panel directly from the ribbon UI. Set the `IsAIButtonVisible` property to `true` to display this button.

```C#
this.radSpreadsheetRibbonBar1.IsAIButtonVisible = true;
this.radSpreadsheetRibbonBar1.AssociatedSpreadsheet = this.radSpreadsheet1;
```
```VB.NET
Me.radSpreadsheetRibbonBar1.IsAIButtonVisible = True
...
```C#
radSpreadsheet1.IsAIEnabled = true;
...
IChatClient chatClient = new ChatClientBuilder(azureClient.AsIChatClient())
    .UseFunctionInvocation()
    .Build();

// Create and assign the AI tools provider
var aiToolsProvider = new AIToolsProvider(chatClient, radSpreadsheet1);
radSpreadsheet1.AIToolsProvider = aiToolsProvider;
```
```VB.NET
radSpreadsheet1.IsAIEnabled = True
...
Dim chatClient As IChatClient = New ChatClientBuilder(azureClient.AsIChatClient()) _
    .UseFunctionInvocation() _
    .Build()

' Create and assign the AI tools provider
Dim aiToolsProvider = New AIToolsProvider(chatClient, radSpreadsheet1)
radSpreadsheet1.AIToolsProvider = aiToolsProvider
```
...
```C#
var aiToolsProvider = new AIToolsProvider(chatClient, this.radSpreadsheet1);
aiToolsProvider.MaxTokenCount = 50000;
this.radSpreadsheet1.AIToolsProvider = aiToolsProvider;
```
```VB.NET
Dim aiToolsProvider = New AIToolsProvider(chatClient, Me.radSpreadsheet1)
aiToolsProvider.MaxTokenCount = 50000
Me.radSpreadsheet1.AIToolsProvider = aiToolsProvider
```

## See Also

* [Getting Started]({%slug radspreadsheet-getting-started%})
* [RadSpreadsheetRibbonBar]({%slug radspreadsheet-getting-started-spreadsheet-ui%})
