...
This tutorial will show you how to bind __RadScheduler__ from the Visual Studio design time surface, by only using the __RadScheduler__ design-time support, and without writing any code. The tutorial uses the data base which comes with the installation of Telerik UI for for WinForms.
...
8\. In the "Choose a Data Connection" page of the wizard, add a connection to the SchedulerData.mdf file supplied with Telerik UI for for WinForms distribution.
  ...
  - Set the Database File Name to the Schedulerdata.mdb file located in the Telerik UI for for WinForms installation directory under \Examples\DataSources.
