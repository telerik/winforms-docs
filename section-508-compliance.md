...
|(f) Textual information shall be provided through operating system functions for displaying text. The minimum information that shall be made available is text content, text input caret location, and text attributes.|Not Supported. Progress&reg; Telerik&reg; UI for WinForms uses the .NET framework functions for displaying text.||
...
|(b) Accessibility and Compatibility Features|Fully Supported: Progress&reg; Telerik&reg; UI for WinForms has accompanying Help documentation listing accessibility and compatibility features. An individual may access and view Help topics online or may select a topic for print.|
