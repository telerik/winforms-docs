---
title: Telerik CLI
page_title: Telerik CLI - Installation, Setup, and Command Reference for Telerik UI for WinForms
...
slug: winforms/getting-started/telerik-cli
...
The Telerik CLI is a .NET global tool that automates common Telerik development tasks from the command line. Use the Telerik CLI to configure the Telerik NuGet package source, create new projects from Telerik templates, and update Telerik UI for WinForms packages without opening Visual Studio.
...
| `telerik setup winforms` | Runs the full Telerik developer-machine setup in one command. | Run to authenticate, configure the Telerik NuGet feed, download your license key, and register the Telerik MCP server configuration. |
...
The `setup` command runs the following steps in sequence to prepare your environment for Telerik UI for WinForms:
...
```text
Telerik UI for WinForms setup complete! Your environment is configured and ready.
```
...
* Specify which Telerik products you are interested in, for example, just `winforms`.
...
To [add the Telerik NuGet server to your package sources]({%slug winforms/nuget%}), use the `nuget config` command:
...
* [Install Telerik UI for WinForms with NuGet]({%slug winforms/nuget%})
* [First Steps with Telerik UI for WinForms]({%slug winforms/getting-started/first-steps%})
* [Restoring NuGet Packages in Your CI Workflow]({%slug install-nuget-keys%})
