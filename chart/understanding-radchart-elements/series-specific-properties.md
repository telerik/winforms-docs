...
slug: winforms/chart-(obsolete)/understanding-radchart-elements/series-specific-properties
...
The __LineSeriesAppearance______property is specific to the [Line]({%slug winforms/chart-(obsolete)/understanding-radchart-types/line-charts%}) and [Spline]({%slug winforms/chart-(obsolete)/understanding-radchart-types/spline-charts%}) chart types.  __LineSeriesAppearance__ has a __Cap__ property that governs the appearance of a line terminating shape that occurs where at each data point (except the first).  Valid __Cap__ values are __Flat__, __Square__, __Round__, __Triangle__, __NoAnchor__, __SquareAnchor__, __RoundAnchor__, __DiamondAnchor__, __ArrowAnchor__, __AnchorMask__and __Custom__. __LineSeriesAppearance__ also has sub properties for __Color__, __PenStyle__, __Visible__ and __Width__.
